...
// ========================================
let SCRIPT_URL = localStorage.getItem('SCRIPT_URL') || 'https://script.google.com/macros/s/AKfycbxnNV8BZWDkZfZC0kN6crKGCxmli39rM2x8c176tJRrjoXJALvypkMKFwL2iijnrseK/exec';
...
document.getElementById('absenForm').addEventListener('submit', async function(e) {
    ...
    // VALIDASI 3: Absen Masuk (cek duplikat)
    if (tipeAbsen === 'Masuk') {
        if (checkAbsenMasukHariIni(absenDateInput)) {
            console.error('❌ Validation failed: sudah absen masuk hari ini');
            showAlert('error', 'Anda sudah melakukan absen masuk pada tanggal tersebut!');
            return;
        }
    }

    // VALIDASI 4: Izin (butuh alasan)
    if (tipeAbsen === 'Izin' && !alasan) {
        console.error('❌ Validation failed: izin tanpa alasan');
        showAlert('error', 'Mohon isi alasan izin!');
        return;
    }

    // VALIDASI 5: Pulang (butuh jenis pulang & scope)
    if (tipeAbsen === 'Pulang') {
        console.log('Validasi Pulang - jenisPulang:', jenisPulang);
        console.log('Validasi Pulang - scopePekerjaan:', scopePekerjaan);
        
        if (!jenisPulang) {
            console.error('❌ Validation failed: jenis pulang tidak dipilih');
            console.error('Radio buttons:', document.querySelectorAll('input[name="jenisPulang"]'));
            showAlert('error', 'Mohon pilih jenis pulang (Normal atau Lembur)!');
            return;
        }
        if (!scopePekerjaan) {
            console.error('❌ Validation failed: scope pekerjaan kosong');
